...
@section('content')
    ...
    <main id="main-content" class="px-4 sm:px-6 lg:px-8 py-6">
        ...
        @php
            $isExpired = $notice->expire_at ? \Carbon\Carbon::parse($notice->expire_at)->isPast() : false;
            ...
            <div class="xl:col-span-2 space-y-5 animate-fadeUp">
                ...
                <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10 relative overflow-hidden">
                    ...
                    <div class="flex items-start justify-between gap-4 mt-1">
                        ...
                        <div class="flex-1">
                            <div
                                class="inline-flex items-center gap-2 bg-yellow-400/10 border border-yellow-400/20 px-3 py-1.5 rounded-full text-xs text-yellow-400">
                        ...
                        <div
                            class="w-14 h-14 rounded-2xl {{ $p['bg'] }} {{ $p['text'] }} flex items-center justify-center border {{ $p['border'] }} shrink-0">
                    ...
                    <div class="flex flex-wrap gap-2 mt-5">
                        ...
                        <span
                            class="inline-flex items-center gap-1.5 {{ $p['bg'] }} {{ $p['text'] }} {{ $p['border'] }} border text-xs px-3 py-1.5 rounded-full">
                            <span
                                class="w-2 h-2 rounded-full {{ $p['dot'] }} {{ $notice->priority === 'urgent' ? 'animate-pulse' : '' }}"></span>
                            {{ $p['label'] }} Priority
                        </span>

                        @if ($notice->category)
                            <span
                                class="inline-flex items-center gap-1.5 bg-input border border-bdr text-sec text-xs px-3 py-1.5 rounded-full">
                        ...
                        @if ($notice->is_scrolling)
                            <span
                                class="inline-flex items-center gap-1.5 bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 text-xs px-3 py-1.5 rounded-full">
                        ...
                        @if ($isExpired)
                            <span
                                class="inline-flex items-center gap-1.5 bg-red/10 text-red border border-red/20 text-xs px-3 py-1.5 rounded-full">
                ...
                <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10">
                    ...
                    <div class="flex items-center gap-3 mb-4">
                        <div
                            class="w-9 h-9 rounded-xl bg-yellow-400/10 border border-yellow-400/20 text-yellow-400 flex items-center justify-center">
                            <i class="fas fa-file-alt text-sm"></i>
                        </div>
                        <h2 class="text-white font-semibold text-white">Notice Content</h2>
                    </div>
                ...
                @if ($notice->attachment)
                    <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <div
                                class="w-9 h-9 rounded-xl bg-accent/10 border border-accent/20 text-accent flex items-center justify-center">
                                <i class="fas fa-eye text-sm"></i>
                            </div>
                            <h2 class="text-white font-semibold text-prim">Preview</h2>
                        </div>

                        <div
                            class="bg-input border border-bdr rounded-2xl px-5 py-4 flex items-center justify-between gap-4">

                           <img class="w-100" src="{{ asset('storage/'.   $notice->attachment) }}" alt="Notice Image">
                    ...
                    <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10">
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div
                                    class="w-10 h-10 rounded-xl bg-accent/10 border border-accent/20 text-accent flex items-center justify-center shrink-0">
                            ...
                            <a href="{{ asset('storage/notice/' . $notice->assignment) }}" target="_blank" download
            ...
            <div class="space-y-5 animate-fadeUp">
                ...
                <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10">
                    ...
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <h2 class="text-white font-semibold text-prim">Notice Info</h2>
                    </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="bg-input border border-bdr rounded-2xl px-4 py-3">
                            ...
                            @if ($notice->is_scrolling)
                                <p class="text-sm font-semibold text-yellow-400"><i class="fas fa-check-circle mr-1"></i>
                                    Enabled</p>
                            @else
                                <p class="text-sm font-semibold text-sec"><i class="fas fa-times-circle mr-1"></i> Disabled
                                </p>
                            @endif
                        </div>
                ...
                @if ($notice->expire_at && !$isExpired)
                    <div class="bg-card border border-bdr rounded-3xl p-6 shadow-lg shadow-black/10">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <div
                                class="w-9 h-9 rounded-xl bg-yellow-500/10 border border-yellow-500/20 text-yellow-400 flex items-center justify-center">
                                <i class="fas fa-hourglass-half text-sm"></i>
                            </div>
                            <h2 class="text-white font-semibold text-prim">Expires In</h2>
                        </div>
